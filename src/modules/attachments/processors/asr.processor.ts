...
@Injectable()
export class AsrProcessor implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    this.worker = new Worker(
      ...
      async (job: Job<{ attachmentId: string; storageKey: string; kind: string }>) => {
        ...
        // Stub: simulate ASR processing (Whisper-large-v3 for Hausa)
        const transcript = `ASR_STUB: Hausa transcription from ${storageKey}`;
            ...
            async (c) => {
              ...
              await c.query(
                `UPDATE attachments
                 SET processing_state = 'done',
                     ...
                     processing_meta = jsonb_set(processing_meta, '{asr}', $3::jsonb)
                 WHERE id = $4`,
                [transcript, confidence, JSON.stringify({ provider: 'stub-whisper', model: 'large-v3', language: 'ha' }), attachmentId],
              );

              // Also insert a field_set op so the transcription becomes part of the report canonical
              const payload = JSON.stringify({
                key: `asr_${kind}_${attachmentId}`,
                value: transcript,
                source: 'voiced',
                confidence,
              });
              await c.query(
                `INSERT INTO report_op_log
                 ...
                 SELECT report_id, $1, 'asr-worker', $2,
