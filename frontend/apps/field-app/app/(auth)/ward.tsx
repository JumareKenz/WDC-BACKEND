...
import { mockWards } from '../../src/lib/mock-lgas';

export default function WardScreen() {
  const router = useRouter();
  const { lgaId } = useLocalSearchParams<{ lgaId: string }>();
  const t = useFormatMessage();
  const [selected, setSelected] = useState<string | null>(null);

  const wards = mockWards[lgaId] ?? [];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>{t('auth.selectWard')}</Text>
        <Text style={styles.subtitle}>Choose your Ward</Text>
      </View>
